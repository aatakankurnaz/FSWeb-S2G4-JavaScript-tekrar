...
let tekrarSayilari = {}
  for (let i of sayilar) {
    tekrarSayilari[i] ? tekrarSayilari[i]++ : tekrarSayilari[i] = 1
  }
  for (let i in tekrarSayilari) {
    if ( tekrarSayilari[i] > 1) {
    tekraredensayilar.push(`${i} sayısı ${tekrarSayilari[i]} kere tekrar edilmiştir`)
  }
 }
